...
def upgrade() -> None:
    ...
    op.execute(
        ...
        WHERE email_normalized IS DISTINCT FROM '2963613812@qq.com'
        """
    )
    op.execute(
        """
        ...
        WHERE email_normalized = '2963613812@qq.com'
        """
    )
    op.execute(
        """
    ...
    op.execute(
        """
        ALTER TABLE career_assistant.platform_users
        ADD CONSTRAINT platform_users_admin_email_check
        CHECK (
            (role = 'admin' AND email_normalized = '2963613812@qq.com')
            OR (role = 'user' AND email_normalized IS DISTINCT FROM '2963613812@qq.com')
        )
        """
    )
    op.execute(
...
def downgrade() -> None:
    ...
    op.execute(
        """
        ALTER TABLE career_assistant.platform_users
        DROP CONSTRAINT platform_users_admin_email_check
        """
    )
    op.execute(
        """
