...
def get_online_assessment_service(request: Request) -> Iterator[OnlineAssessmentService]:
    ...
    service = OnlineAssessmentService(
        ...
        vision_client=services.cloud_vision_client,
    )
    try:
        yield service
    finally:
...
def stream_assessment_solution(
    ...
    def events():
        ...
        try:
            for event in service.solve_events(body.problem):
                if event.type == "correcting":
                    yield _ndjson(
                        {"type": "phase", "phase": "correcting", "run_version": body.run_version}
                    )
                elif event.type == "needs_confirmation":
                    yield _ndjson(
                        {
                    ...
                    return
                elif event.type == "solution" and event.solution is not None:
                    yield _ndjson(
                        {
                            "type": "solution",
                            "run_version": body.run_version,
                            "solution": event.solution.model_dump(mode="json"),
                        }
                    )
