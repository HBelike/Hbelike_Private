...
def normalize_capture(capture: CapturedProblemInput) -> AssessmentProblem:
    ...
    if not statement:
        statement = "未识别到题目正文"
    problem_type = _infer_problem_type(capture, statement)
    language = _resolve_language(capture.language_hint, problem_type)
    ...
    if problem_type is ProblemKind.ALGORITHM and language is ProgrammingLanguage.UNKNOWN:
        incomplete_reasons.append("无法识别编程语言")
    if problem_type is ProblemKind.ALGORITHM and interface_kind is InterfaceKind.UNKNOWN:
        incomplete_reasons.append("无法判断题目输入输出接口")
    if problem_type is ProblemKind.ALGORITHM and interface_kind is InterfaceKind.FUNCTION and not function_signature:
        incomplete_reasons.append("函数题缺少函数签名")
    if (
        problem_type is ProblemKind.ALGORITHM
        and interface_kind is InterfaceKind.FUNCTION
        and language in {ProgrammingLanguage.JAVA, ProgrammingLanguage.CPP}
        and function_signature
        and not _signature_has_static_types(function_signature)
    ):
        incomplete_reasons.append("Java/C++ 函数题缺少参数类型")
    if problem_type is ProblemKind.CHOICE and len(capture.choice_candidates) < 2:
        incomplete_reasons.append("选择题缺少完整选项")
...
def _best_statement(capture: CapturedProblemInput) -> str:
    ...
    if candidates:
        if capture.capture_mode.endswith("_adapter_v2"):
            return candidates[0]
...
def _infer_problem_type(capture: CapturedProblemInput, statement: str) -> ProblemKind:
    hint = capture.question_type_hint.strip().lower().replace("-", "_")
    aliases = {
        "algorithm": ProblemKind.ALGORITHM,
        "coding": ProblemKind.ALGORITHM,
        "sql": ProblemKind.SQL,
        "choice": ProblemKind.CHOICE,
        "multiple_choice": ProblemKind.CHOICE,
        "short_answer": ProblemKind.SHORT_ANSWER,
        "text": ProblemKind.SHORT_ANSWER,
    }
    if hint in aliases:
        return aliases[hint]
    if capture.starter_code or capture.function_signature:
        return ProblemKind.ALGORITHM
    lowered = statement.lower()
    if re.search(r"\b(select|insert|update|delete|join|group\s+by)\b", lowered) and re.search(
        r"\b(sql|table|from|where|数据库|数据表|查询)\b", lowered
    ):
        return ProblemKind.SQL
    if len(re.findall(r"(?m)^\s*[A-HＡ-Ｈ][\.、:：\)]\s*\S+", statement)) >= 2:
        return ProblemKind.CHOICE
    if re.search(r"(?:简述|解释|为什么|谈谈|describe|explain)", statement, re.IGNORECASE):
        return ProblemKind.SHORT_ANSWER
    return ProblemKind.ALGORITHM


def _resolve_language(hint: str, problem_type: ProblemKind) -> ProgrammingLanguage:
    if problem_type is ProblemKind.SQL:
        return ProgrammingLanguage.SQL
    if problem_type in {ProblemKind.CHOICE, ProblemKind.SHORT_ANSWER}:
        return ProgrammingLanguage.TEXT
    normalized = re.sub(r"[^a-z+#]", "", hint.strip().lower())
...
def _signature_has_static_types(signature: str) -> bool:
    arguments = re.search(r"\(([^)]*)\)", signature)
    if not arguments:
        return False
    value = arguments.group(1).strip()
    if not value:
        return True
    if ":" in value:
        return True
    return bool(re.search(r"\b(?:int|long|double|float|bool|boolean|char|string|vector|list|map|set|array)\b", value, re.IGNORECASE))
...
def _normalize_platform(value: str) -> str:
    ...
    supported = {"leetcode", "hackerrank", "nowcoder", "codesignal", "codility", "saima"}
...
def _confidence(
    *,
    ...
    if has_examples:
        score += 0.10
    if problem_type is not ProblemKind.ALGORITHM:
        score += 0.35
