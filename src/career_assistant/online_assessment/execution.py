...
class PistonExecutionProvider:
    ...
    def execute(
        ...
        *,
        ...
        if solution.language not in _RUNTIME_NAMES:
            raise ValueError("当前答案语言不能进入代码执行器")
        if problem is not None and solution.language is not problem.language:
            raise ValueError("答案语言与题目选择不一致，请重新生成后再测试")
