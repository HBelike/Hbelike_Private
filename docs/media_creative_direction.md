...
2026-09-02 修复 `content_id=21` 的 4 层架构图渲染失败：根因是同层双节点的长说明文字超出节点边界，Gotenberg 最终以 503 超时表现。模板现在对 4～5 层启用紧凑密度、6 层及以上启用高密度，并将模板版本升级为 `article_visual_v2`，避免复用旧 CSS 生成的图片。针对原始 6 份视觉规格的真实 ImageTask 首轮生成 6 张、第二轮按 `render_key` 复用 6 张；6 个有效资产均通过 DOM 校验且为 `2048×1152` PNG。文章排版预检确认 6 个本地图片占位符、6 个可上传文件和封面素材齐全；相关自动化测试共 120 项通过。本次未调用付费图片模型、微信 API 或执行生产部署。
