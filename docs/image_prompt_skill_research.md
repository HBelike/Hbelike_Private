...
2. `ImageTask` 用 `ImagePromptDesignService` 把结构化简报编译成不超过 300 个汉字的最终 Prompt。长度上限来自[火山方舟图片生成教程](https://docs.volcengine.com/docs/82379/1824121)的官方建议：
   - 风格：技术教学讲义、白底、商务技术 PPT、扁平化 2D 矢量；
   - 形状：规整矩形模块，黑色实线正交折线箭头，严格禁止曲线与交叉线；
   - 颜色：黑白灰为主，少量蓝色点缀，不使用 3D、装饰和花哨渐变；
   - 文字：只允许 `visual_brief` 指定的模块名和连线标签，不生成标题、正文、页码、仓库名或额外说明；
   - 信息分工：详细项目解释放在图片下方的文章图注，图片只承担组件边界、数据方向和耦合关系。

## 为什么不让模型直接生成带文字架构图

Seedream 可以生成中文短标签，但不能保证复杂段落逐字正确。项目因此只保留最多 4 个模块名和必要连线标签，并明确禁止标题与正文段落：
