...
**Goal:** 将平台角色收敛为 `admin/user`，固定 `2963613812@qq.com` 为唯一管理员，并恢复普通用户按管理员模块配置访问页面的能力。
...
**Interfaces:**
- Produces: `PLATFORM_ADMIN_EMAIL: str = "2963613812@qq.com"`
...
def test_migration_normalizes_roles_and_pins_admin_email(self) -> None:
    ...
    self.assertIn("2963613812@qq.com", source)
    self.assertIn("role IN ('user', 'admin')", source)
    self.assertIn("role <> 'admin' OR email_normalized = '2963613812@qq.com'", source)
```
...
Expected: `2963613812@qq.com|admin|t`；其他账号均为 `user`；admin 数量为 1。
