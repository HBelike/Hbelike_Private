...
- [ ] **Step 3: 人工审计权限、来源与动作白名单**
  ...
  Run: `rg -n 'host_permissions|externally_connectable|allowed|ping|list_cities|search_jobs|get_job_detail|zhipin.com|xingxingtech.cn' browser-extension/job-library`
...
- [ ] **Step 1: 检查服务与健康接口**
  ...
  Run: `docker compose --env-file .env.production -f docker-compose.production.yml ps` 与 `curl -fsS https://xingxingtech.cn/api/health`
