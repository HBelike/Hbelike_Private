...
2. **代码链路**：本地、GitHub `main` 与服务器 `/home/ubuntu/apps/Hbelike_Private` 使用同一发布链路；本机通过独立 GitHub SSH key 推送，服务器通过只读 Deploy Key 拉取，本轮将同步登录门户、免费模型目录与生产视频开关改造。
...
12. **当前修复/下一步**：登录页已形成登录、注册、邮箱验证码和找回密码入口；求职助手模型选择器只展示真实可用模型，免费目录附官方注册地址并区分 Provider/Model ID；前端仅维护电脑端与手机端。当前发布任务是将这些本地改动同步 GitHub/生产、开启真实付费 Seedance，并用生产 readiness、容器状态与公网健康检查验收。
