...
- “公开信息收集”输入关键词和 5–50 条数量后，通过 Firecrawl 搜索全网无需登录的公开网页，完成地址/正文去重、解析、Agent 甄别和自动入库，不依赖浏览器扩展或小红书登录态。
- 小红书浏览器关键词接口仍保留为兼容能力，但不再是 PC 面经库“公开信息收集”的默认实现。

“公开信息收集”默认目标为 20 条、最多 50 条。只有高置信度、公司/岗位/具体问题齐全的完整面经自动入库；其余网页保留为候选或过滤结果。跨关键词先按组织与规范 URL 去重，再按规范正文 SHA-256 去重。
