...
- 视口规则：页面按浏览器高度自动请求会话数量，`950px` 及以下显示 5 条，`951px` 至 `1240px` 显示 7 条，更高视口显示 9 条。窗口跨越阈值后延迟 `160ms` 重新分页，并根据旧页首条记录的位置计算新页码，避免用户浏览位置无故跳回开头。
- 后端契约：继续复用 `GET /api/career/conversations?page=<page>&page_size=<5|7|9>` 的现有分页响应，不修改数据库或 API Schema。新建、发送、重命名、归档和删除仍沿用原有排序与页码修正规则；并发请求继续通过请求序号丢弃过期响应。
- PC 交互：移除“每页数量”和“跳转到页码”两组低频控件，底部只保留当前区间、页数和前后翻页；会话菜单仅在悬停、选中、键盘聚焦或展开时显示。隐私边界压缩为单行说明，不再占用独立卡片高度。
- 依赖与边界：复用 Vue、现有 FastAPI 分页和主题变量，不增加组件库或本地存储项。本次先完成 PC 端密度与键盘焦点，手机端继续使用既有纵向页面和列表滚动规则，不做平板专项适配。
- 验证：21 项 Node 测试与 Vite production build 通过；浏览器实测 `900px`、`1080px`、`1440px` 三种高度分别请求 5、7、9 条，所有普通会话行高均为 `72px`，列表无额外内部溢出。
