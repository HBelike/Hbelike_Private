...
from src.career_assistant.online_assessment.solution_service import SolveProgressEvent
...
class FakeAssessmentService:
    ...
    def solve_events(self, value):
        del value
        yield SolveProgressEvent(type="correcting", reasons=("code:string_too_short",))
...
def test_solve_stream_returns_versioned_ndjson() -> None:
    ...
    assert response.status_code == 200
    assert '"phase":"correcting"' in lines[1]
    assert '"type":"solution"' in lines[2]
    assert '"run_version":2' in lines[2]
