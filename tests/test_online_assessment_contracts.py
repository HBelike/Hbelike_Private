...
def test_deep_adapter_candidate_priority_beats_long_generic_page_noise() -> None:
    problem = normalize_capture(CapturedProblemInput(
        source_platform="nowcoder",
        capture_mode="nowcoder_adapter_v2",
        adapter_key="nowcoder",
        problem_candidates=[
            "题目：数组求和。输入格式：两个整数。输出格式：它们的和。",
            "导航、推荐题目与页脚噪声。" * 200,
        ],
        language_hint="Python 3",
    ))

    assert problem.statement.startswith("题目：数组求和")
    assert "页脚噪声" not in problem.statement
...
def test_unknown_language_is_not_silently_changed_to_python() -> None:
    problem = normalize_capture(
        CapturedProblemInput(
            source_platform="nowcoder",
            problem_candidates=["给定一个整数数组，返回其中最大的元素。输入输出要求见平台函数模板。"],
            function_signature="maxValue(nums)",
        )
    )

    assert problem.source_platform == "nowcoder"
    assert problem.language is ProgrammingLanguage.UNKNOWN
    assert "无法识别编程语言" in problem.incomplete_reasons


def test_sql_and_choice_questions_use_non_execution_routes() -> None:
    sql = normalize_capture(CapturedProblemInput(
        problem_candidates=["SQL：表 Employee(id, salary)，查询第二高的薪水，返回 SecondHighestSalary。"],
        question_type_hint="sql",
    ))
    choice = normalize_capture(CapturedProblemInput(
        problem_candidates=["以下哪项属于 HTTP 状态码？\nA. 200\nB. TCP\nC. JSON\nD. DNS"],
        question_type_hint="choice",
        choice_candidates=["A. 200", "B. TCP", "C. JSON", "D. DNS"],
    ))

    assert sql.problem_type is ProblemKind.SQL
    assert sql.language is ProgrammingLanguage.SQL
    assert sql.interface_kind is InterfaceKind.UNKNOWN
    assert "无法判断题目输入输出接口" not in sql.incomplete_reasons
    assert choice.problem_type is ProblemKind.CHOICE
    assert choice.language is ProgrammingLanguage.TEXT
    assert choice.needs_confirmation is False
