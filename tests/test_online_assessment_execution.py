...
def test_javascript_assignment_function_name_is_not_misread_as_function_keyword() -> None:
    requests: list[dict[str, object]] = []

    def handler(request: httpx.Request) -> httpx.Response:
        requests.append(json.loads(request.content))
        return httpx.Response(200, json={"run": {"stdout": "2", "stderr": "", "code": 0}})
    ...
    problem = AssessmentProblem(
        title="搜索插入位置",
        statement="返回目标值应插入的位置。",
        language=ProgrammingLanguage.JAVASCRIPT,
        interface_kind=InterfaceKind.FUNCTION,
        function_signature="searchInsert = function(nums, target)",
        confidence=1,
    )
    candidate = AssessmentSolution(
        approach_markdown="二分查找。",
        code="searchInsert = function(nums, target) { return 2; };",
        language=ProgrammingLanguage.JAVASCRIPT,
    ...
    provider.execute(
        candidate,
        (AssessmentTestCase(input_payload=[[1, 3, 5, 6], 5], expected_output="2"),),
        problem=problem,
    )

    source = requests[0]["files"][0]["content"]
    assert 'const __oaName = "searchInsert"' in source


def test_execute_rejects_solution_language_different_from_problem() -> None:
    provider = PistonExecutionProvider("http://piston.test")
    problem = AssessmentProblem(
        title="求和",
    ...
    with pytest.raises(ValueError, match="语言与题目选择不一致"):
        provider.execute(
            solution(ProgrammingLanguage.PYTHON),
            (AssessmentTestCase(input_payload="1 2", expected_output="3"),),
            problem=problem,
        )
