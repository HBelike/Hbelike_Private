...
import { assessmentCanvasSections, isAssessmentPending } from '../career-assessment-view.js'
    ...
    <div v-if="hasStructuredCanvas" class="job-main-grid">
      <section class="job-section job-primary-section">
        ...
        <ol v-if="responsibilities.length" class="job-points">
          <li v-for="(item,index) in responsibilities.slice(0,4)" :key="`responsibility-${index}`"><i>{{ index+1 }}</i><p>{{ item }}</p></li>
        </ol>
      ...
      <section class="job-section job-primary-section">
        ...
        <div v-if="requirementEntries.length" class="job-requirements">
          <article v-for="(item,index) in requirementEntries.slice(0,4)" :key="`requirement-${index}`" :class="item.type"><i></i><div><small>{{ item.type }}</small><p>{{ item.text }}</p></div></article>
        </div>
