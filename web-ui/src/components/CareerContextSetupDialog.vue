...
async function parseResume() {
  ...
  try {
    ...
    resumePreview.value = await requestJson('/api/career/candidate-profiles/parse', {
      method: 'POST',
      body: formData
    })
  } catch (error) {
    ...
    <div v-if="open" class="context-backdrop" @mousedown.self="cancel">
      <section class="context-dialog" role="dialog" aria-modal="true" aria-label="建立求职上下文">
        <header class="context-dialog-header">
          ...
          <button type="button" aria-label="关闭" :disabled="busy" @click="cancel">×</button>
        </header>
        ...
        <main class="context-dialog-body">
          <section v-if="step === 1" class="context-step-panel">
            ...
            <div class="mode-switch">
              <button type="button" :class="{ active: resumeMode === 'existing' }" :disabled="!candidateProfiles.length" @click="chooseResumeMode('existing')">使用已有简历</button>
              <button type="button" :class="{ active: resumeMode === 'upload' }" @click="chooseResumeMode('upload')">上传新简历</button>
            </div>
            ...
            <div v-else class="upload-workbench">
              ...
              <button class="file-drop" type="button" @click="resumeInput?.click()">
                <span>↑</span><strong>{{ resumeFile?.name || '选择一份基准简历' }}</strong><small>PDF、Word 或清晰图片，最大 10MB</small>
              </button>
              <button v-if="resumeFile && !resumePreview" class="parse-button" type="button" :disabled="parsingResume" @click="parseResume">{{ parsingResume ? '正在读取简历…' : '解析简历' }}</button>
        ...
        <footer class="context-dialog-footer">
          ...
          <div class="footer-actions">
            <button v-if="step === 1" type="button" class="secondary" :disabled="busy" @click="skipResume">{{ initialContext?.candidate_profile ? '保留当前简历' : '暂不添加简历' }}</button>
...
.context-dialog-header h2{margin:3px 0 7px;font-size:26px;letter-spacing:-.04em}.context-dialog-header p{margin:0;color:#647695}.context-kicker{color:#0870df!important;font:800 11px/1.2 ui-monospace,SFMono-Regular,Consolas,monospace;letter-spacing:.14em}.context-dialog-header>button{display:grid;width:36px;height:36px;place-items:center;border:1px solid #d4e0f0;border-radius:50%;background:#fff;color:#657792;font-size:23px;cursor:pointer}
