...
const busy = computed(() => ['capturing', 'analyzing', 'generating', 'correcting', 'testing', 'repairing'].includes(phase.value))
...
const canSolve = computed(() => Boolean(
  problem.value?.statement
  && !busy.value
  && (!isExecutableProblem.value || problem.value?.language !== 'unknown')
))
const currentLanguage = computed(() => solution.value?.language || problem.value?.language || 'unknown')
const isExecutableProblem = computed(() => (problem.value?.problem_type || 'algorithm') === 'algorithm')
const answerEditorLabel = computed(() => {
  if (problem.value?.problem_type === 'sql') return 'SQL 答案 · 可编辑'
  if (problem.value?.problem_type === 'choice') return '选项答案 · 可编辑'
  if (problem.value?.problem_type === 'short_answer') return '参考答案 · 可编辑'
  return `${currentLanguage.value} · 可编辑`
})
const pipelineSteps = computed(() => {
  const order = ['analyzing', 'generating', 'testing']
  const pipelinePhase = phase.value === 'repairing' ? 'testing' : phase.value === 'correcting' ? 'generating' : phase.value
  ...
  try {
    await assessmentApi.solve(
      ...
      (event) => {
        ...
        if (event.type === 'phase' && event.phase === 'correcting') {
          syncManager({ phase: 'correcting' }, runVersion)
          noticeMessage.value = '模型已生成内容，正在自动校正答案格式。'
        }
        if (event.type === 'solution') {
          syncManager({ phase: isExecutableProblem.value ? 'testing' : 'ready', solution: event.solution }, runVersion)
        }
    ...
    if (!isExecutableProblem.value) {
      const nextItems = upsertAssessmentItem(assessmentItems.value, {
        problem: problem.value,
        solution: solution.value,
        report: null
      })
      syncManager({ phase: 'ready', items: nextItems }, runVersion)
      noticeMessage.value = problem.value?.problem_type === 'sql'
        ? 'SQL 答案已生成；当前未连接数据库，因此未宣称执行通过。'
        : '参考答案已生成，请结合题目要求复核。'
      return
    }
...
function handleLanguageChange() {
  ...
  noticeMessage.value = '语言已切换，旧答案与测试已清除。请重新生成对应语言的答案。'
}

function handleProblemTypeChange() {
  if (problem.value?.problem_type === 'sql') problem.value.language = 'sql'
  if (['choice', 'short_answer'].includes(problem.value?.problem_type)) problem.value.language = 'text'
  if (problem.value?.problem_type === 'algorithm' && ['sql', 'text'].includes(problem.value.language)) {
    problem.value.language = 'unknown'
  }
...
async function detectExtension() {
  ...
  try {
    ...
    extensionReady.value = ping.capabilities?.includes('online_assessment_capture_v2') === true
    ...
    <section v-else class="assessment-workbench" :aria-busy="busy">
      <article class="assessment-panel problem-panel">
        ...
        <div v-if="problem" class="problem-editor">
          ...
          <div class="problem-contract">
            <label><span>题型</span><select v-model="problem.problem_type" :disabled="busy" @change="handleProblemTypeChange"><option value="algorithm">编程题</option><option value="sql">SQL</option><option value="choice">选择题</option><option value="short_answer">简答题</option></select></label>
            <label><span>语言</span><select v-model="problem.language" :disabled="busy" @change="handleLanguageChange"><option value="unknown">待识别</option><option value="python">Python</option><option value="javascript">JavaScript</option><option value="java">Java</option><option value="cpp">C++</option><option value="sql">SQL</option><option value="text">文本作答</option></select></label>
      ...
      <article class="assessment-panel solution-panel">
        ...
        <div v-else class="code-skeleton" aria-live="polite">
          ...
          <p>{{ phase === 'analyzing' ? '先判断输入输出与约束' : phase === 'correcting' ? '正在校正模型返回格式' : phase === 'generating' ? '正在组织思路并生成完整代码' : '题面确认后将在这里生成代码' }}</p>
        </div>
      </article>
      ...
      <article class="assessment-panel tests-panel">
        <header>
          ...
          <button type="button" :disabled="!solution || busy || !isExecutableProblem" @click="executeCurrent(null, false)">重新测试</button>
        </header>
        ...
        <div v-else class="test-empty">
          <span aria-hidden="true">▤</span>
          <p>{{ isExecutableProblem ? '公开样例与 AI 边界测试会在代码生成后自动运行。' : '当前题型生成参考答案，不进入代码执行器。' }}</p>
        </div>
