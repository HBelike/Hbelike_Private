...
<template>
  <Teleport to="body">
    <div
      ...
      @keydown.esc="cancel"
    >
      <section class="career-job-dialog" role="dialog" aria-modal="true" aria-labelledby="career-job-dialog-title">
        <header class="career-job-dialog-header">
          <div>
            <nav aria-label="当前位置"><span>求职助手</span><i>/</i><strong>职位检索</strong></nav>
            <h2 id="career-job-dialog-title">选择当前会话的目标岗位</h2>
          </div>
...
.career-job-dialog{display:grid;width:min(1640px,calc(100vw - 48px));height:calc(100dvh - 48px);min-height:620px;grid-template-rows:auto minmax(0,1fr) auto auto;overflow:hidden;border:1px solid var(--ui-line-strong,#b9cfed);border-radius:22px;background:var(--ui-surface,#fff);box-shadow:0 32px 100px rgba(5,35,78,.3);color:var(--ui-text,#14213d);animation:career-job-dialog-in .16s ease-out}
.career-job-dialog-header{display:flex;align-items:center;justify-content:space-between;gap:24px;border-bottom:1px solid var(--ui-line,#d7e4f6);background:linear-gradient(135deg,var(--ui-surface,#fff),var(--ui-surface-active,#eaf3ff));padding:15px 20px}.career-job-dialog-header nav{display:flex;align-items:center;gap:7px;color:var(--ui-text-muted,#7b879c);font:750 10px/1.3 var(--ui-font-utility,Consolas,monospace)}.career-job-dialog-header nav i{font-style:normal}.career-job-dialog-header nav strong{color:var(--ui-accent-ink,#004aa8)}.career-job-dialog-header h2{margin:4px 0 0;color:var(--ui-text,#14213d);font:850 20px/1.25 var(--ui-font-display,"Segoe UI Variable Display",sans-serif);letter-spacing:-.025em}.career-job-dialog-header>button{display:grid;width:36px;height:36px;place-items:center;border:1px solid var(--ui-line,#d7e4f6);border-radius:10px;background:var(--ui-surface,#fff);color:var(--ui-text-secondary,#526078);font-size:24px;line-height:1;cursor:pointer}.career-job-dialog-header>button:hover:not(:disabled){border-color:var(--ui-accent,#0869d8);color:var(--ui-accent-ink,#004aa8)}
