...
export function captureToApiPayload(capture = {}) {
  return {
    ...
    capture_contract_version: Number(capture.captureContractVersion) || 2,
    adapter_key: String(capture.adapterKey ?? 'generic').slice(0, 80),
    adapter_version: String(capture.adapterVersion ?? '2').slice(0, 40),
    field_sources: capture.fieldSources && typeof capture.fieldSources === 'object' ? capture.fieldSources : {},
    question_type_hint: String(capture.questionTypeHint ?? '').slice(0, 80),
    choice_candidates: Array.isArray(capture.choiceCandidates) ? capture.choiceCandidates.slice(0, 20) : []
  }
}
...
export function mergeAssessmentCaptures(previous = {}, incoming = {}) {
  ...
  return {
    ...
    languageHint: incoming.languageHint || previous.languageHint || '',
    functionSignature: incoming.functionSignature || previous.functionSignature || '',
    questionTypeHint: incoming.questionTypeHint || previous.questionTypeHint || '',
    choiceCandidates: unique([
      ...(previous.choiceCandidates ?? []),
      ...(incoming.choiceCandidates ?? [])
    ]).slice(0, 20),
